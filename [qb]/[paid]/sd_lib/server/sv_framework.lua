...
local function ValidateResource(source)
    ...
    if not SD.utils.Contains(Secure.AllowedResources, invokingResource) then
        local playerId = SD.GetIdentifier(source) or "Unknown ID"
        local playerName = SD.GetName(source) or "Unknown Name"

        SD.KickPlayer(source, "Unauthorized resource access detected.")
        print("^1ERROR: Resource '" .. invokingResource .. "' attempted unauthorized access by player: ".. playerName .. " with ID: " .. playerId .. ".^0")
...
SD.RegisterCallback('sd_bridge:getIdentifier', function(source, target, cb)
    -- Call the cb function with the identifier of the target player
    cb(SD.GetIdentifier(target))
end)
