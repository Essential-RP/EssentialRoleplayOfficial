...
RegisterNetEvent('essential-storerobbery:client:mzRegisterHit', function()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "essential-storerobbery-register",          -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-90",
...
RegisterNetEvent('essential-storerobbery:client:mzSafeHit', function()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "essential-storerobbery-safe",          -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-90",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
...
RegisterNetEvent('essential-storerobbery:client:mzLiquorHit', function()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "essential-storerobbery-liquor",          -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-90",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
...
function setupRegister()
    QBCore.Functions.TriggerCallback('essential-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    QBCore.Functions.TriggerCallback('essential-storerobbery:server:getSafeStatus', function(Safes)
...
function StealFromRegister(k)
    QBCore.Functions.TriggerCallback('essential-storerobbery:server:getCops', function(cops)
            ...
            if not Config.UseGabz then
                if not Config.RegistersTarget[k].robbed then
                    if cops >= Config.MinimumStoreRobberyPolice then
                        currentRegister = k
                        print(k)
                        TriggerServerEvent('essential-storerobbery:server:setRegisterStatus', currentRegister)
                        ...
                        elseif Config.BreakRegister == "circle" then 
                            TriggerEvent('essential-storerobbery:client:circleLockpickAdvanced')
                            if Config.psdispatch then 
                                if not copsCalled then 
                                    TriggerEvent('essential-storerobbery:client:mzRegisterHit')
                                    copsCalled = true
                        ...
                        if not Config.psdispatch then 
                            if not copsCalled then
                                ...
                                TriggerServerEvent("essential-storerobbery:server:callCops", "cashier", currentRegister, streetLabel, pos)
                                copsCalled = true
                            end
                        end 
                    else
        ...
        elseif QBCore.Functions.HasItem("advancedlockpick") and QBCore.Functions.HasItem("screwdriverset") then
            if not Config.UseGabz then 
                if not Config.RegistersTarget[k].robbed then
                    if cops >= Config.MinimumStoreRobberyPolice then
                        currentRegister = k
                        ...
                        if Config.BreakRegister == "standard" then 
                            print('Config.BreakRegister == "standard" is no longer supported, please select "circle"')
                        elseif Config.BreakRegister == "circle" then 
                            TriggerEvent('essential-storerobbery:client:circleLockpick')
                            if Config.psdispatch then 
                                if not copsCalled then 
                                    TriggerEvent('essential-storerobbery:client:mzRegisterHit')
                                    copsCalled = true
            ...
            -- }
            QBCore.Functions.Notify('Might Need Something A Little More Advanced', "error", 3500)
            if Config.NotifyType == 'qb' then
...
RegisterNetEvent('essential-storerobbery:client:circleLockpick', function()
        ...
        if success then
            if currentRegister ~= 0 then
                ...
                QBCore.Functions.Progressbar("search_register", "Emptying the till...", lockpickTime, false, true, {
                    ...
                    registerDone = false 
                    TriggerServerEvent('essential-storerobbery:server:takeMoney', currentRegister, true, registerDone)
                    registerDone = true
                    Wait(1000)
            ...
            TriggerServerEvent('essential-storerobbery:server:setRegisterStatusFailed', currentRegister)
            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
            if Config.NotifyType == 'qb' then
            ...
            if math.random(1, 100) <= Config.LockpickBreakChance then
                TriggerServerEvent("essential-storerobbery:server:RemoveLockpick")
...
RegisterNetEvent('essential-storerobbery:client:circleLockpickAdvanced', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"uncuff"})
    TriggerServerEvent("essential-storerobbery:server:RemoveAdvanced", 1)
    exports['ps-ui']:Circle(function(success)
        if success then
            if currentRegister ~= 0 then
                openingDoor = true 
...
RegisterNetEvent('essential-storerobbery:client:CheckXPLevel', function()
    lvl8 = false
    lvl7 = false
    lvl6 = false
    lvl5 = false
            ...
            if not Config.SafesTarget[safe].robbed then
                if cops >= Config.MinimumStoreRobberyPolice then
                    ...
                    if Config.SafesTarget[safe].type == "keypad" then -- CONVENIENCE STORE PARTICULARS (NON-GABZ CONFIGURATION)
                        if QBCore.Functions.HasItem(Config.SafeReqItem.tool) and QBCore.Functions.HasItem(Config.SafeReqItem.sc) then
                            TriggerServerEvent('essential-storerobbery:server:setSafeStatus', currentSafe)
                            if Config.psdispatch then 
                                TriggerEvent('essential-storerobbery:client:mzSafeHit')
                            else 
                                if not copsCalled then
                                    ...
                                    TriggerServerEvent("essential-storerobbery:server:callCops", "safe", currentSafe, streetLabel, pos)
                                    copsCalled = true
                                end
                            end 
                            TriggerEvent('essential-storerobbery:client:CheckXPLevel')
                            if Config.Hacktype == "varhack" then 
                                TriggerServerEvent("essential-storerobbery:server:setSafeStatus", currentSafe)
                                TriggerEvent('animations:client:EmoteCommandStart', {"kneel2"})
                                Wait(500)
                                TriggerEvent('animations:client:EmoteCommandStart', {"mechanic"})
                                TriggerServerEvent("essential-storerobbery:server:ItemRemoval")
                                if Config.mzskills then
                            ...
                            QBCore.Functions.Notify('Might Need Something Used To Crack Safes', "error", 3500)
                            -- TriggerEvent('inventory:client:requiredItems', requiredItems, true)
                            Wait(3500)
                            -- TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        end 
                        ...
                        if QBCore.Functions.HasItem(Config.LiquorReqItem) then
                            if Config.psdispatch then 
                                TriggerEvent('essential-storerobbery:client:mzLiquorHit')
                            else 
                                if not copsCalled then
                                    pos = GetEntityCoords(PlayerPedId())
                                    local s1, s2 = GetStreetNameAtCoord(pos.x, pos.y, pos.z)
                            ...
                            TriggerServerEvent('essential-storerobbery:server:setSafeStatus', currentSafe)
                            Wait(250)
                            TriggerServerEvent("essential-storerobbery:server:ItemRemovalLiquor")
                            TriggerEvent('essential-storerobbery:client:CheckXPLevel')
...
function ExchangeSuccessSafe() 
    ...
    QBCore.Functions.Progressbar("deliver_reycle_package", "Robbing safe...", saferobtime, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        safeCheck = false
        TriggerServerEvent("essential-storerobbery:server:SafeReward", currentSafe, safeCheck)
        safeCheck = true
        ClearPedTasks(PlayerPedId())
        Wait(1000)
        if Config.mzskills then  
...
function ExchangeSuccessSafeLiquor() 
    ...
    QBCore.Functions.Progressbar("deliver_reycle_package", "Raiding safe...", saferobtime, false, true, {
        ...
        safeCheck = false
        TriggerServerEvent("essential-storerobbery:server:SafeRewardAlcohol", currentSafe, safeCheck)
        safeCheck = true
        ClearPedTasks(PlayerPedId())
        Wait(1000)
        if Config.mzskills then  
...
function ExchangeFailSafe(safe)
    TriggerEvent('animations:client:EmoteCommandStart', {"c"})
    TriggerServerEvent("essential-storerobbery:server:SafeFail")
    TriggerServerEvent("essential-storerobbery:server:setSafeStatusFailed", currentSafe)
    Wait(1000)
    if Config.StressEnabled then 
...
RegisterNetEvent('essential-storerobbery:client:setRegisterStatus', function(k, bool)
    if not Config.UseGabz then 
...
RegisterNetEvent('essential-storerobbery:client:setSafeStatus', function(safe, bool)
    if not Config.UseGabz then
...
RegisterNetEvent('essential-storerobbery:client:robberyCall', function(_, _, _, coords)
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorouter1", vector3(-1224.33, -911.06, 12.33), 1.2, 0.18, {
        ...
        maxZ = 13.53,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorOuter1",
                icon = 'fas fa-key',
...
RegisterNetEvent('essential-storerobbery:client:LiquorOuter1', function()
        ...
        exports['ps-ui']:Circle(function(success)
            if success then
                ...
                if math.random(1, 100) <= Config.BreakChance then 
                    TriggerServerEvent('essential-storerobbery:server:KeyRemoval')
                else
                    if Config.NotifyType == 'qb' then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorinner1", vector3(-1220.22, -912.72, 12.33), 1.2, 0.15, {
        ...
        maxZ = 13.53,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorInner1",
                icon = 'fas fa-key',
...
RegisterNetEvent('essential-storerobbery:client:LiquorInner1', function()
    if QBCore.Functions.HasItem(Config.LiquorItem) then
        TriggerEvent('animations:client:EmoteCommandStart', {"knock2"})
        exports['ps-ui']:Circle(function(success)
            if success then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorboth1", vector3(-1216.95, -915.99, 11.33), 0.4, 0.2, {
        ...
        maxZ = 12.33,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorBoth1",
...
RegisterNetEvent('essential-storerobbery:client:LiquorBoth1', function()
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorouter2", vector3(-1483.25, -379.75, 40.16), 1.2, 0.15, {
        ...
        maxZ = 41.36,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorOuter2",
                icon = 'fas fa-key',
                label = 'Unlock Outer door'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorOuter2', function()
    if QBCore.Functions.HasItem(Config.LiquorItem) then
        TriggerEvent('animations:client:EmoteCommandStart', {"knock2"})
        exports['ps-ui']:Circle(function(success)
            if success then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorinner2", vector3(-1482.29, -375.48, 40.16), 1.2, 0.15, {
        ...
        maxZ = 41.36,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorInner2",
                icon = 'fas fa-key',
                label = 'Unlock Storeroom'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorInner2', function()
    if QBCore.Functions.HasItem(Config.LiquorItem) then
        TriggerEvent('animations:client:EmoteCommandStart', {"knock2"})
        exports['ps-ui']:Circle(function(success)
            if success then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorboth2", vector3(-1479.69, -371.6, 39.16), 0.5, 0.2, {
        ...
        maxZ = 40.16,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorBoth2",
                icon = 'fas fa-hand-scissors',
                label = 'Trip Locks'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorBoth2', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"mechanic4"})
    triplocks = math.random(Config.TripLocksLow, Config.TripLocksHigh)
    QBCore.Functions.Progressbar("deliver_reycle_package", "Tripping locks...", (triplocks * 1000), false, true, {
        disableMovement = true,
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorouter3", vector3(-2965.68, 387.46, 15.04), 1.2, 0.15, {
        ...
        maxZ=16.44,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorOuter3",
                icon = 'fas fa-key',
                label = 'Unlock outer door'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorOuter3', function()
    if QBCore.Functions.HasItem(Config.LiquorItem) then
        TriggerEvent('animations:client:EmoteCommandStart', {"knock2"})
        exports['ps-ui']:Circle(function(success)
            if success then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorinner3", vector3(-2961.88, 389.61, 15.04), 1.2, 0.15, {
        ...
        maxZ=16.24,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorInner3",
                icon = 'fas fa-key',
                label = 'Unlock Storeroom'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorInner3', function()
    if QBCore.Functions.HasItem(Config.LiquorItem) then
        TriggerEvent('animations:client:EmoteCommandStart', {"knock2"})
        exports['ps-ui']:Circle(function(success)
            if success then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorboth3", vector3(-2957.27, 390.24, 14.04), 0.4, 0.2, {
        ...
        maxZ=15.04,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorBoth3",
                icon = 'fas fa-hand-scissors',
                label = 'Trip Locks'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorBoth3', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"mechanic4"})
    triplocks = math.random(Config.TripLocksLow, Config.TripLocksHigh)
    QBCore.Functions.Progressbar("deliver_reycle_package", "Tripping locks...", (triplocks * 1000), false, true, {
        disableMovement = true,
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorouter4", vector3(1169.32, 2711.74, 38.16), 1.2, 0.15, {
        ...
        maxZ=39.36,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorOuter4",
                icon = 'fas fa-key',
                label = 'Unlock outer door'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorOuter4', function()
    if QBCore.Functions.HasItem(Config.LiquorItem) then
        TriggerEvent('animations:client:EmoteCommandStart', {"knock2"})
        exports['ps-ui']:Circle(function(success)
            if success then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorinner4", vector3(1166.94, 2715.46, 38.16), 1.2, 0.15, {
        ...
        maxZ=39.36,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorInner4",
                icon = 'fas fa-key',
                label = 'Unlock Storeroom'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorInner4', function()
    if QBCore.Functions.HasItem(Config.LiquorItem) then
        TriggerEvent('animations:client:EmoteCommandStart', {"knock2"})
        exports['ps-ui']:Circle(function(success)
            if success then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorboth4", vector3(1166.02, 2720.0, 37.16), 0.5, 0.2, {
        ...
        maxZ=38.16,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorBoth4",
                icon = 'fas fa-hand-scissors',
                label = 'Trip Locks'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorBoth4', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"mechanic4"})
    triplocks = math.random(Config.TripLocksLow, Config.TripLocksHigh)
    QBCore.Functions.Progressbar("deliver_reycle_package", "Tripping locks...", (triplocks * 1000), false, true, {
        disableMovement = true,
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorouter5", vector3(1132.86, -979.28, 46.42), 1.2, 0.15, {
        ...
        maxZ=47.62,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorOuter5",
                icon = 'fas fa-key',
                label = 'Unlock outer door'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorOuter5', function()
    if QBCore.Functions.HasItem(Config.LiquorItem) then
        TriggerEvent('animations:client:EmoteCommandStart', {"knock2"})
        exports['ps-ui']:Circle(function(success)
            if success then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorinner5", vector3(1129.51, -982.11, 46.42), 1.2, 0.15, {
        ...
        maxZ=47.62,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorInner5",
                icon = 'fas fa-key',
                label = 'Unlock Storeroom'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorInner5', function()
    if QBCore.Functions.HasItem(Config.LiquorItem) then
        TriggerEvent('animations:client:EmoteCommandStart', {"knock2"})
        exports['ps-ui']:Circle(function(success)
            if success then
...
CreateThread(function()
    exports['qb-target']:AddBoxZone("liquorboth5", vector3(1125.1, -983.63, 45.42), 0.5, 0.2, {
        ...
        maxZ=46.42,
            ...
            {
                type = "client",
                event = "essential-storerobbery:client:LiquorBoth5",
                icon = 'fas fa-hand-scissors',
                label = 'Trip Locks'
            },
        },
...
RegisterNetEvent('essential-storerobbery:client:LiquorBoth5', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"mechanic4"})
    triplocks = math.random(Config.TripLocksLow, Config.TripLocksHigh)
    QBCore.Functions.Progressbar("deliver_reycle_package", "Tripping locks...", (triplocks * 1000), false, true, {
        disableMovement = true,
