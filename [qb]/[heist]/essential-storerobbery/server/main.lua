...
QBCore.Functions.CreateCallback('essential-storerobbery:server:getCops', function(source, cb)
    local amount = 0
...
RegisterNetEvent('essential-storerobbery:server:setRegisterStatus', function(k)
    if not Config.UseGabz then 
        Config.RegistersTarget[k].robbed = true
        Config.RegistersTarget[k].time = Config.resetTime
        TriggerClientEvent('essential-storerobbery:client:setRegisterStatus', -1, k, true)
        SetTimeout(Config.resetTime, function()
            Config.RegistersTarget[k].robbed = false
            TriggerClientEvent('essential-storerobbery:client:setRegisterStatus', -1, k, false)
        end)
    else
...
RegisterNetEvent('essential-storerobbery:server:setRegisterStatusFailed', function(k)
    if not Config.UseGabz then 
        Config.RegistersTarget[k].robbed = false
        TriggerClientEvent('essential-storerobbery:client:setRegisterStatus', -1, k, false)
    else 
...
RegisterNetEvent('essential-storerobbery:server:setSafeStatus', function(safe)
    if not Config.UseGabz then 
        Config.SafesTarget[safe].robbed = true
        TriggerClientEvent('essential-storerobbery:client:setSafeStatus', -1, safe, true)
        SetTimeout(Config.SafeResetTime, function()
            Config.SafesTarget[safe].robbed = false
            TriggerClientEvent('essential-storerobbery:client:setSafeStatus', -1, safe, false)
        end)
    else 
...
RegisterNetEvent('essential-storerobbery:server:setSafeStatusFailed', function(safe)
    if not Config.UseGabz then
        Config.SafesTarget[safe].robbed = false
        TriggerClientEvent('essential-storerobbery:client:setSafeStatus', -1, safe, false)
    else 
...
RegisterNetEvent('essential-storerobbery:server:takeMoney', function(register, isDone, registerDone)
    if not registerDone then 
        ...
        if isDone then
            if Config.CashRegisterReturn == "rolls" then 
                local amount = math.random(Config.minRegisterEarn, Config.maxRegisterEarn)
                Player.Functions.AddItem('rolls', amount)
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['rolls'], "add", amount)
        ...
        print("Someone is attempting to trigger 'essential-storerobbery:server:takeMoney' externally.")
    end
end)

-- CONVENIENCE STORES

RegisterNetEvent('essential-storerobbery:server:SafeReward', function(safe, safeCheck)
    if not safeCheck then 
        ...
        if Config.SafeReturn == "rolls" then 
        ...
        print("Someone is attempting to trigger 'essential-storerobbery:server:SafeReward' externally.")
    end
end)

-- LIQUOR STORES

RegisterNetEvent('essential-storerobbery:server:SafeRewardAlcohol', function(safe, safeCheck)
    if not safeCheck then 
        local src = source
        local Player = QBCore.Functions.GetPlayer(src)
        if not Player then 
        ...
        if Config.AlcoholReturn == "rolls" then 
            local amount = math.random(Config.AlcoholminSafeEarn, Config.AlcoholmaxSafeEarn)
            Player.Functions.AddItem('rolls', amount, false)
        ...
        print("Someone is attempting to trigger 'essential-storerobbery:server:SafeRewardAlcohol' externally.")
    end
end)
...
RegisterServerEvent('essential-storerobbery:server:ItemRemoval', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.RemoveItem(Config.SafeReqItem.tool, 1)
    Player.Functions.RemoveItem(Config.SafeReqItem.sc, 1)
	TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.SafeReqItem.sc], "remove", 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.SafeReqItem.tool], "remove", 1)
end)

RegisterServerEvent('essential-storerobbery:server:ItemRemovalLiquor', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
-- RegisterServerEvent('essential-storerobbery:server:RemoveLockpick', function()
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('essential-storerobbery:server:RemoveAdvanced', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('essential-storerobbery:server:SafeFail', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Config.NotifyType == 'qb' then
...
RegisterServerEvent('essential-storerobbery:server:KeyRemoval', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('essential-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId
    if type == "safe" then
    ...
    TriggerClientEvent("essential-storerobbery:client:robberyCall", -1, type, safe, streetLabel, coords)
...
CreateThread(function()
    while true do
        ...
        if #toSend > 0 then
            TriggerClientEvent('essential-storerobbery:client:setRegisterStatus', -1, toSend, false)
        end
        Wait(Config.tickInterval)
    end
end)

QBCore.Functions.CreateCallback('essential-storerobbery:server:getPadlockCombination', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

QBCore.Functions.CreateCallback('essential-storerobbery:server:getRegisterStatus', function(_, cb)
    if not Config.UseGabz then 
...
QBCore.Functions.CreateCallback('essential-storerobbery:server:getSafeStatus', function(_, cb)
    if not Config.UseGabz then 
