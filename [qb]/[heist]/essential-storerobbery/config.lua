...
Config.NotifyType = 'qb-core'                      -- Notification type (set to 'qbcore' to use qb-core standard notifications.)
...
Config.DispatchRegisterDelay = 30               -- Time (in seconds) between dispatch pings being sent when lockpicking registers
...
Config.LockpickBreakChance = 100                 -- Percentage chance of lockpick breaking upon player failing to lockpick cash register.
Config.AdvancedBreakChance = 100                 -- Percentage chance of advanced lockpick breaking upon player failing to lockpick cash register.
...
Config.circleparses = 6                         -- number of circle parses a player will need to complete
Config.circletime = 20                          -- time for player to complete one parse of the circle

-- Reward type:
Config.CashRegisterReturn = "rolls"        -- Set to "dirtymoney", "markedbills" or "cash" to alter the reward given.
Config.minRegisterEarn = 25                   -- Minimum amount earnt from stealing from a cash register
Config.maxRegisterEarn = 35                   -- Maximum amount earnt from stealing from a cash register

Config.RegisterTime = 120                        -- progressBar time it takes for player to rob cash register after lockpicking (in seconds)
...
Config.SafeTimelow = 180                         -- Lowest time (in seconds) that it will take a player to raid the safe.
Config.SafeTimehigh = 300                        -- Highest time (in seconds) that it will take a player to raid the safe.

Config.SafeResetTime = 60 * 1000 * 30           -- The first number is the time, in minutes, for a safe to cooldown automatically. 
...
Config.SafeReqItem = {
    sc = "safecracker",
    tool = "screwdriverset",   
}               -- Change to any item you want (needs to be in qb-core/shared/items.lua)


Config.SafeReturn = "rolls"                -- Set to "rolls", "markedbills" or "cash" to alter the reward given.

Config.minSafeEarn = 35                      -- Minimum amount earnt from stealing from a safe (in dirtymoney)
Config.maxSafeEarn = 50                      -- Maximum amount earnt from stealing from a safe (in dirtymoney)
...
Config.RareItem1Chance = 35                     -- Percentage chance to obtain the rare item drop from robbing a safe.
...
Config.RareItem2 = "goldchain"                    -- Name of the rare item that will drop
Config.RareItem2Chance = 15                     -- Percentage chance to obtain the rare item drop from robbing a safe.
Config.RareItem2Amount = 3                      -- Amount of the rare item player will receive if chance is triggered
--Rare item 3 (Note: chance for drop is independent for each rare item listed)
Config.RareItem3 = "purpleusb"              -- Name of the rare item that will drop
Config.RareItem3Chance = 3                      -- Percentage chance to obtain the rare item drop from robbing a safe.
...
Config.AlcoholReturn = "rolls"             -- Set to "rolls", "markedbills" or "cash" to alter the reward given.

Config.AlcoholminSafeEarn = 25                -- Minimum amount earnt from stealing from a safe (in dirtymoney)
Config.AlcoholmaxSafeEarn = 40               -- Maximum amount earnt from stealing from a safe (in dirtymoney)
Config.AlcoholSafeTime = 240                     -- Time it takes for player to rob safe (in seconds)
...
--Rare item 2 (Note: chance for drop is independent for each rare item listed)
Config.AlcoholRareItem2 = "goldchain"             -- Name of the rare item that will drop
Config.AlcoholRareItem2Chance = 10              -- Percentage chance to obtain the rare item drop from robbing a safe.
Config.AlcoholRareItem2Amount = 2               -- Amount of the rare item player will receive if chance is triggered
--Rare item 3 (Note: chance for drop is independent for each rare item listed)
Config.AlcoholRareItem3 = "weapon_pistol"       -- Name of the rare item that will drop
Config.AlcoholRareItem3Chance = 0.5               -- Percentage chance to obtain the rare item drop from robbing a safe.
