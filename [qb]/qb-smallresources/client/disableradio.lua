...
--disable peds spawning mlos

Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)  -- Wait for 1 second to reduce script load
        
        local playerPed = PlayerPedId()
        local playerCoords = GetEntityCoords(playerPed)
        
        -- Check if player is inside a building
        if IsInteriorEntitySetActive(playerCoords.x, playerCoords.y, playerCoords.z) then
            local nearbyPeds = GetNearbyPeds(playerCoords.x, playerCoords.y, playerCoords.z, 50.0)
            
            for _, ped in ipairs(nearbyPeds) do
                -- Check if the ped is not the player and is an AI ped
                if ped ~= playerPed and not IsPedAPlayer(ped) then
                    DeletePed(ped)  -- Remove the ped
                end
            end
        end
    end
