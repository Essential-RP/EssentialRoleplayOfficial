...
-- Config for items you want the Ped in the cafeteria to sell.
CanteenItems = {
    [1] = {
        name = "sandwich",
        price = 50,
...
RegisterNetEvent("Thx:Server:UpdateFoodItemsAmount", function(type,amount,name)
    if type == 'remove' then
        for k,v in pairs(CanteenItems) do
            if v.name == name then
                CanteenItems[k].amount = CanteenItems[k].amount - amount
            end
        end
    else
        local random = math.random(1, #CanteenItems)
        for k,v in pairs(CanteenItems) do
            if random == k then
                CanteenItems[k].amount = CanteenItems[k].amount + 1
            end
        end
    end
end)
...
QBCore.Functions.CreateCallback('Thx:server:CantienAmount', function(_, cb)
    cb(CanteenItems)
end)
