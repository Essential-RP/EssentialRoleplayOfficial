...
---==[[ PREVIEW BOARD ITEM ]]==---
	RegisterNetEvent("jim-mechanic:server:giveList", function(info)
		...
		if Config.System.Inv == "ox" then
			...
			if Config.Overrides.showItemBox then TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["mechboard"], "add", 1) end
		end
	end)
...
if not Config.Overrides.disableNos then
	...
	RegisterNetEvent('jim-mechanic:server:SyncFlame', function(netId, enabled, antilag)
		local nearbyList = GetNearbyPeds(NetworkGetEntityFromNetworkId(netId), Config.NOS.FlameDis or 30.0)
		for i = 1, #nearbyList do TriggerClientEvent('jim-mechanic:client:SyncFlame', nearbyList[i], netId, enabled, antilag) end
	end)
...
RegisterNetEvent('jim-mechanic:server:updateCar', function(netId, props) TriggerClientEvent("jim-mechanic:forceProperties", -1, netId, props) end)
