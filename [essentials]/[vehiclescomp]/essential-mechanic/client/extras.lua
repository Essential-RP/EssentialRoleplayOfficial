...
local function updateOdo(dist, veh, plate)
	...
	if Config.System.Debug then
		...
		if VehicleNitrous[plate] and Config.Overrides.disableNos then
...
RegisterNetEvent("jim-mechanic:Client:ExitVehicle", function()
	...
	if seat == -1 then
		...
		if DoesEntityExist(veh) then
			SetVehicleHandlingFloat(veh, "CHandlingData","fMass", currentVehicle["hFloat"])
			SetVehicleCheatPowerIncrease(veh, currentVehicle["pIncrease"])
			SetVehicleTurboPressure(veh, currentVehicle["tPressure"])
			ModifyVehicleTopSpeed(veh, currentVehicle["speedMod"])
		end
		currentVehicle = {}
	end
	seat = nil
...
RegisterNetEvent("jim-mechanic:Client:EnteredVehicle", function()
	...
	currentVehicle = { -- Grab original values for NOS
		["hFloat"] = GetVehicleHandlingFloat(veh, "CHandlingData", "fMass"),
		["pIncrease"] = GetVehicleCheatPowerIncrease(veh),
		["tPressure"] = GetVehicleTurboPressure(veh),
		["speedMod"] = GetVehicleTopSpeedModifier(veh),
	...
	CreateThread(function()
		if Config.Harness.HarnessControl == true then
			while GetVehiclePedIsIn(Ped, false) == veh do
				if VehicleStatus[plate]["carwax"] ~= 0 then	SetVehicleDirtLevel(veh, 0.0) WashDecalsFromVehicle(vehicle, 1.0) end -- if carwax, clean car
	...
	if GetSeatPedIsIn(veh) == -1 then
				...
				if not IsVehicleStopped(GetVehiclePedIsIn(Ped)) then
					...
					if DistAdd >= 3000 then
						...
						local MechStatus = {
							...
							["carwax"] = VehicleStatus[plate].carwax or false,
						}
						if Config.System.Debug then print("^5Debug^7: ^2Writing ^3VehicleStatus ^2to database^7") end
						TriggerServerEvent('jim-mechanic:server:saveStatus', MechStatus, plate, GetVehicleEngineHealth(veh), GetVehicleBodyHealth(veh))
		...
		CreateThread(function()
			while GetSeatPedIsIn(veh) == -1 do
				...
				local wait = Config.System.Debug == true and 0 or 200
				Wait((wait))
			end
		end)
