...
$(document).on('click', '#IconClassStyle', function(evt){
    ...
    if (type !== "lab") {
        ...
        $.post('https://essential-spawn/setCam', JSON.stringify({
            posname: location,
            type: type,
        }));
...
$(document).on('click', '.GreenBTN', function(evt){
    ...
    if (Mainlocation !== null){
        ...
        if (MainspawnType == "appartment2") {
            $.post('https://essential-spawn/spawnplayerappartment2', JSON.stringify({
                spawnloc: Mainlocation,
                apartName: apartNames,
            }));
        } else if(MainspawnType !== "appartment"){
            $.post('https://essential-spawn/spawnplayer', JSON.stringify({
                spawnloc: Mainlocation,
                typeLoc: MainspawnType
            }));
        }else {
            $.post('https://essential-spawn/chooseAppa', JSON.stringify({
...
function CloseAddCoord() {
    $.post('https://essential-spawn/CloseAddCoord', JSON.stringify({}));
